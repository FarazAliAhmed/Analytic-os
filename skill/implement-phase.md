...
> **The difference:** Objective acceptance criteria from your spec. Implementation executes against these criteria. When a task is done, you know it's done because it meets the spec's success criteria.
...
| No | Work with AI to refine until it meets spec |
...
Agent completes Task 1.1, shows output: `[Introduction text, 623 words]`
...
```
You direct. AI executes. You validate. You iterate if needed.
This cycle repeats for each phase.
```
...
1. Task completes
2. You check: "Does it meet success criteria from spec?"
3. Not yet? "AI, adjust X aspect and try again"
4. Meets spec? "Commit. Next task"
...
| Without Iteration (Wrong) | With Iteration (Correct) |
|---------------------------|--------------------------|
| You: "That's too long. Project failed." | You: "This exceeds the 700-word spec limit. Can you condense to 500-700 while keeping all three arguments?" |
...
AI proposes approach: "I'll structure this with (1) research population,
(2) data collection method, (3) analysis approach. Does this align with
what you're aiming for?"

You refine: "Add one more: (4) validity checks. Want to ensure readers
can replicate this."

AI adjusts proposal: "Perfect. Here's revised structure with validity
checks integrated..."
...
| Sources span theoretical, empirical, contrasting | ✓ |
...
| When to use | Your role |
|-------------|-----------|
| Linear projects (writing, sequential analysis) | Validate after each task block, ensure quality before next block uses output |
...
| When to use | Your role |
|-------------|-----------|
| Independent sections (research paper: intro, methodology, analysis can draft in parallel) | Ensure parallel tasks don't create integration conflicts at checkpoint |
...
| When to use | Your role |
|-------------|-----------|
| Complex tasks, creative work (good first draft exists, needs polish) | Provide specific feedback ("expand argument 2," "add example," "tighten analysis") |
...
| Category | Questions |
|----------|-----------|
| **Success Criteria Met?** | Does output match explicit criteria from task definition? Word count? Format? Tone? Completeness? |
| **Specification Requirements Fulfilled?** | Does this task's output advance toward spec's overall success evals? Are we building the right thing? |
| **Quality Standards?** | Does output meet your constitutional standards (from Lesson 3)? Academic rigor? Clarity? Coherence? |
| **Ready for Next Task?** | Can the next task safely build on this output? Any risks or dependencies that aren't met? |
...
**What happens:** Agent refines that task, re-executes, shows updated output. You review again.
...
| ❌ WRONG | ✓ RIGHT |
|----------|---------|
| Agent: "Task complete"<br>You: "Looks good" (without checking against spec) | Agent: "Task complete"<br>You: [Spend 2-5 minutes validating against spec] |
...
| ❌ WRONG | ✓ RIGHT |
|----------|---------|
| Spec says: 500-700 words<br>Task delivers: 780 words<br>You: "Close enough, let's move on" | Spec says: 500-700 words<br>Task delivers: 780 words<br>You: "Over spec limit. AI, condense to 500-700 while keeping key points." |
...
| ❌ WRONG | ✓ RIGHT |
|----------|---------|
| Agent: "Task 2.1 complete"<br>You: "This doesn't meet spec"<br>Agent: "Too bad. Moving on."<br>You: "Okay, I guess project is compromised now" | Agent: "Task 2.1 complete"<br>You: "This doesn't meet spec. Here's the gap: [specific feedback]"<br>Agent: "Understood. Adjusting and re-executing..."<br>Agent: [Shows revised output]<br>You: [Validates again]<br>Result: Task now meets spec |
...
The Implement Phase brings your specification, plan, and tasks to life through structured collaboration. By using checkpoints, validating against specifications, and embracing iteration, you maintain control while leveraging AI's execution capabilities.

**Key Takeaways:**

1. Implementation ≠ autonomous execution—it's orchestrated collaboration
2. Checkpoints keep you in control and catch issues early
3. Spec success criteria are your acceptance standard
4. Iteration loops accelerate problem-solving when first passes fail
5. AI collaborates—not just executes, but suggests, adapts, and refines
6. Three checkpoint decisions: Commit/Proceed, Iterate, or Revise Plan
...
- `/sp.git.commit_pr` — Commit at each checkpoint
- `/sp.specify` — Create new specification for next project
