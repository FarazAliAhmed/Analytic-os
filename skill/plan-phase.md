...
| Input | Output |
|-------|--------|
| Your specification (what the paper must accomplish) | Implementation plan with: Architecture overview, Implementation phases, Component breakdown, Dependencies and sequencing, Design decisions highlighted |
...
In your AI tool, from your research-paper directory:
...
| Skip ADR For | Reason |
|--------------|--------|
| Style choices | Formatting preferences, no long-term impact |
| Obvious choices | Already decided by Constitution (APA citation style) |
| Temporary decisions | Will revisit before submission |
| Out-of-scope decisions | Already decided by spec |
...
## Context
We need to decide when research happens relative to writing: research-first (.
Two approaches existgather all sources, then write)
...
|-------|--------|-------------------|
| **Specification** | spec.md | "What is success?" (Measurable criteria) |
| **Plan** | plan.md | "How will we organize the work?" (Architecture + ADRs) |
| **Tasks** | tasks.md | "What are the 15-30 minute units?" (Atomic work) |

**Example chain:**

```
Specification says:
"Write a 3000-5000 word research paper on AI in K-12 education,
APA format, 8+ sources, define 3+ concrete applications"

Plan says:
"Structure: Research Phase → Foundation Writing → Analysis Writing → Synthesis.
Phases in order, dependencies mapped. Key decision: research-concurrent approach."

Tasks (next lesson) will say:
"Task 1: Define thesis (15 min), Task 2: Outline all sections (30 min),
Task 3: Research for Lit Review (45 min)..." etc.
...
| Question | If NO → Skip ADR |
|----------|------------------|
| Does this have long-term consequences? | Skip |
| Are there multiple viable alternatives? | Skip |
| Will someone ask "why did we choose this" in 6 months? | Skip |

> **All three must be true** for ADR creation.
...
The Plan Phase transforms your specification into an actionable architecture. By generating a detailed implementation plan and documenting key decisions as ADRs, you ensure that subsequent task generation is based on clear, consistent foundations.

**Key Takeaways:**

1. `/sp.plan` creates implementation architecture from your specification
2. Plans define phases, components, dependencies, and design decisions
3. ADRs document architecturally significant choices with context and rationale
4. Apply the three-part test before creating an ADR
5. Spec → Plan → Tasks form a cascade of increasing specificity
...
- `/sp.clarify` — Refine specification before planning
- `/sp.tasks` — Generate testable tasks after planning
